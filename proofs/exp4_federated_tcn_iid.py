...
# --- CONFIGURATION ---
EXPERIMENT_NAME = "Exp4: Federated TCN (IID)"
...
def run_experiment():
    ...
    if history and history.metrics_distributed:
        ...
        pd.DataFrame([final_round_metrics]).to_csv(os.path.join(BASE_DIR, f"proofs/results/metrics_exp4.csv"), index=False)
        print(f"\nProof saved to proofs/results/metrics_exp4.csv")
    else:
